...
const EditCategorySelector = ({ form }: CategorySelectorProps) => {
  const {
    setValue,
    watch,
    formState: { errors },
  ...
  // Set initial values from form - only once
  useEffect(() => {
    if (watchedCategory) {
      setSelectedCategory(watchedCategory);
      const category = categories.find((c) => c._id === watchedCategory);
      if (category) {
    ...
    if (watchedSubCategory) {
      ...
      const subCategory = subCategories.find(
        (sc) => sc._id === watchedSubCategory
      );
      if (subCategory) {
        setSubCategorySearch(subCategory.name);
      }
    }
  }, [watchedCategory, watchedSubCategory, categories, subCategories]);
  ...
  const getSelectedSubCategoryName = () => {
    if (subCategorySearch) return subCategorySearch;
    const subCategory = subCategories.find(
